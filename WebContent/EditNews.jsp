...
<table width="99%" border="0" cellspacing="0" cellpadding="0" id="searchmain">
  <tr>
    <td width="99%" align="left" valign="top">您的位置：新闻管理&nbsp;&nbsp;&nbsp;&nbsp;编辑新闻</td>
  </tr>
  <tr>
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0" id="main-tab">
      <tr onMouseOut="this.style.backgroundColor='#ffffff'" onMouseOver="this.style.backgroundColor='#edf5ff'">
        ...
        <c:if test="${not empty map && map.containsKey('title')}">
           <input class="text-word" type='text' name='title' id='title' value='${news.title}'/>${map.title }<span id="msg"></span>
        </c:if>
        <c:if test="${empty map || !map.containsKey('title')}">
           <input class="text-word" type='text' name='title' id='title' value='${news.title}'/><span id="msg"></span>
        </c:if>
        </td>
        </tr>
      <tr onMouseOut="this.style.backgroundColor='#ffffff'" onMouseOver="this.style.backgroundColor='#edf5ff'">
        ...
        <select name='typeid'>
          <c:forEach items="${typeList}" var="type">
            <option value="${type.id}" ${news.typeid == type.id ? "selected='selected'" : "" }>${type.typeName}</option>
          </c:forEach>
        </select>
        </td>
      </tr>
      <tr onMouseOut="this.style.backgroundColor='#ffffff'" onMouseOver="this.style.backgroundColor='#edf5ff'">
        ...
        <c:if test="${not empty map && map.containsKey('createtime')}">
           <input class="text-word Wdate" type='text' name='createtime' id='createtime' value='${news.createtime}' onClick="WdatePicker()"/>${map.createtime }
        </c:if>
        <c:if test="${empty map || !map.containsKey('createtime')}">
           <input class="text-word Wdate" type='text' name='createtime' id='createtime' value='${news.createtime}' onClick="WdatePicker()"/>
        </c:if>
        </td>
        </tr>
      <%-- <tr onMouseOut="this.style.backgroundColor='#ffffff'" onMouseOver="this.style.backgroundColor='#edf5ff'">
        ...
        <textarea id="content" name="content">${news.content}</textarea>${map.content }
        </c:if>
      ...
      <tr onMouseOut="this.style.backgroundColor='#ffffff'" onMouseOver="this.style.backgroundColor='#edf5ff'">
        <td align="right" valign="middle" class="borderright borderbottom bggray">审核：</td>
        <td align="left" valign="middle" class="borderright borderbottom main-for">
        <select name='flag'>
          <option value='0' ${news.flag==0 ? "selected='selected'" : ""}>未审核</option>
          <option value='1' ${news.flag==1 ? "selected='selected'" : ""}>已审核</option>
        </select>
        </td>
      </tr>
      </c:if>
      ...
      <tr onMouseOut="this.style.backgroundColor='#ffffff'" onMouseOver="this.style.backgroundColor='#edf5ff'">
        ...
        <td align="left" valign="middle" class="borderright borderbottom main-for">
        <input type='hidden' name='nFlag' value='editNews'/>
        ...
        <input name="" type="submit" value="编辑" class="text-but" id="save" onclick="save()">
