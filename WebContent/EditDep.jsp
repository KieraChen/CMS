	  ...
	  depName.onblur = function(){
		  ...
		  req.onreadystatechange = function(){
			  if(req.status == 200 && req.readyState == 4){
				  ...
				  if(data == "exist"){
					  msg.innerHTML = "<font color='red'>部门名已被占用</font>";
					  sb.disabled = true;
				  }else{
					  msg.innerHTML = "<font color='blue'>部门名可以使用</font>";
					  sb.disabled = false;
				  }
			  }
		  };
...
<table width="99%" border="0" cellspacing="0" cellpadding="0" id="searchmain">
  <tr>
    <td width="99%" align="left" valign="top">您的位置：部门管理&nbsp;&nbsp;&nbsp;&nbsp;编辑部门</td>
  </tr>
  <tr>
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0" id="main-tab">
      <tr onMouseOut="this.style.backgroundColor='#ffffff'" onMouseOver="this.style.backgroundColor='#edf5ff'">
        ...
        <c:if test="${empty map || !map.containsKey('depName')}">
           <input class="text-word" type='text' name='depName' id='depName' value='${dep.depName }'/><span id="msg"></span>
        </c:if>
        </td>
        </tr>
      <tr onMouseOut="this.style.backgroundColor='#ffffff'" onMouseOver="this.style.backgroundColor='#edf5ff'">
        ...
        <c:if test="${empty map || !map.containsKey('depCreateTime')}">
           <input class="text-word Wdate" type='text' name='depCreateTime' value='${dep.depCreateTime }' onClick="WdatePicker()"/>
        </c:if>
        </td>
        </tr>
      <tr onMouseOut="this.style.backgroundColor='#ffffff'" onMouseOver="this.style.backgroundColor='#edf5ff'">
        ...
        <c:if test="${empty map || !map.containsKey('sort')}">
           <input class="text-word" type='text' name='sort' value='${dep.sort}'/>
        </c:if>
        </td>
      </tr>
